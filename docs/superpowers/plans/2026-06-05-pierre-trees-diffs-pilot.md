...
- 运行单测:`pnpm --filter quarry-app exec vitest run <相对 apps/electron 的路径>`
- typecheck:`nx run quarry-app:typecheck`;lint:`nx run quarry-app:lint`
...
```bash
pnpm --filter quarry-app add diff && pnpm --filter quarry-app add -D @types/diff
```
...
Run: `pnpm --filter quarry-app exec vitest run src/renderer/src/lib/pierre-diffs-theme.test.ts`
...
Run: `nx run quarry-app:typecheck`
Expected: 通过,无类型错误。

- [ ] **Step 4: smoke 验证 worker 在 Electron renderer 可用**

Run: `pnpm dev:mock`(或 `nx run quarry-app:dev:mock`),应用启动后打开任意视图,确认控制台**无** worker 初始化报错(如 `Failed to construct 'Worker'`)。
...
Run: `pnpm --filter quarry-app exec vitest run src/renderer/src/lib/sql-diff.test.ts`
...
Run: `pnpm --filter quarry-app exec vitest run src/renderer/src/stores/schema-comparison-store.test.ts`
...
Run: `nx run quarry-app:typecheck && nx run quarry-app:lint`
Expected: 通过。
...
Run: `pnpm --filter quarry-app exec vitest run src/renderer/src/lib/folder-tree-paths.test.ts`
...
typecheck:`nx run quarry-app:typecheck`。若 `useFileTree` 必填字段缺失,读 `node_modules/@pierre/trees/dist/model/publicTypes.d.ts` 的 `FileTreeOptions` 补齐。
...
```bash
pnpm --filter quarry-app exec vitest run            # 全量单测
nx run quarry-app:typecheck
nx run quarry-app:lint
nx run quarry-app:build                              # 受影响时
```
