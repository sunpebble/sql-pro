...
**结论(API 层)**:核心交互(层级/选中/右键/拖拽)可达,但有 3 个明确 ⚠️ 摩擦(双击连接、行内多 badge、展开回写)+ 2 个待 GUI 确认项(空目录、主题)。属「可行但有 UX 回退」,非阻断 ❌、也非全 ✅。最终 go/no-go 需 `pnpm dev:mock` 跑通 Step 6 的 8 项清单后定;在此之前 `FolderTreeView` 保持与旧 `FolderTree` 并存,**不替换 `ProfileManager` 引用**。
