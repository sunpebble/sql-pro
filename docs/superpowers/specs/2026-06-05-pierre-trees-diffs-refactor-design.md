...
Quarry 当前的树状视图与 diff 视图**全部为手写实现**,未使用任何第三方 tree/diff 库。本次重构引入 Pierre Computer Company 的两个库:
...
- **`@pierre/diffs` = 增强,不是替换。** 它渲染的是**文本/代码逐行 diff**(git patch 或两份文件文本 + Shiki 高亮)。而 Quarry 的 `schema-comparison` 是**结构化语义 diff**(`TableDiff → ColumnDiff`,带类型变化、过滤、子对象展开)。二者不是同一种东西;强行替换会丢失过滤/展开/行内编辑等业务交互。`@pierre/diffs` 真正契合的用例是 **SQL 文本的 diff 呈现**,因此采用「新增文本 diff 视图,与结构化视图并存切换」。
...
1. **targeted test**:`nx run quarry-app:test`(改动相关组件)
2. **typecheck**:`nx run quarry-app:typecheck`
3. **lint**:`nx run quarry-app:lint`
4. **build**:`nx run quarry-app:build`(受影响时)
...
## 10. API 校准补遗(基于真实 `.d.ts`,2026-06-05)

安装 `@pierre/trees@1.0.0-beta.4` 与 `@pierre/diffs@1.2.7` 后,从 `node_modules` 真实 `.d.ts` 复核,对 §5 设计作如下校准(以本节为准):

### 10.1 `@pierre/trees` 是 path-first 文件树(取代 §5.2 的 `TreeNode` 模型)

- `useFileTree(options: FileTreeOptions): { model }`;渲染 `<FileTree model={model} renderContextMenu={...} ...hostProps />`。
- 树结构由 **`paths: string[]`** 定义(`/` 分隔推断层级),**不是**嵌套 `children`。其余配置:展开 `initialExpansion: 'closed'|'open'|number` / `initialExpandedPaths`;选择 `initialSelectedPaths` / `onSelectionChange(paths)`;搜索 `search` / `fileTreeSearchMode` / `useFileTreeSearch`;拖拽 `dragAndDrop: { canDrag, canDrop, onDropComplete }`;右键 `renderContextMenu(item: {kind,name,path}, ctx)`;行装饰 `renderRowDecoration → {text}|{icon}`;图标 `icons`;自定义样式 `unsafeCSS`。
- **含义**:它是「成品文件树 + 有限定制点」,不是裸 headless 的逐行 render-prop。因此适配层从「`TreeNode<T>` + `renderNode`」改为:**`buildPaths(data) → string[]` + `Map<path, entity>` 反查 + `renderRowDecoration`/`renderContextMenu` 定制**。
- basename 即显示 label,故 path 段需「唯一且可读」;同级重名用去重后缀,段内 `/` 需转义。
- 这套定制点对 Sidebar 的 row-count badge / pin / 7+ 操作菜单**契合度存疑**,正是 FolderTree 试点要验证、并据以决定是否推广到 Sidebar 的核心。

### 10.2 `@pierre/diffs` 输入(取代 §5.3 的 before/after 直传)

- `<PatchDiff patch={unifiedDiff} options={{ theme }} />` —— 最简路径,`patch` 为 unified diff 字符串。
- 备选:`<FileDiff fileDiff={FileDiffMetadata} />` / `<File file={FileContents} />`;`FileContents = { name, contents, lang?, header?, cacheKey? }`;库导出 `parseDiffFromFile` 可从 `FileContents` 生成。
- **`SqlDiffView` 采用**:jsdiff `createTwoFilesPatch(name, name, before, after)` 生成 patch → `<PatchDiff>`。需新增 `diff` + `@types/diff` 依赖(`@pierre/diffs` 已内部依赖 `diff@8`,此处显式声明)。
- theme:`useThemeStore` 的 `resolvedTheme` → `isDark ? 'pierre-dark' : 'pierre-light'`。
- split/stacked:`PatchDiff` 默认 unified;split 布局以试点中读 `components/FileDiff.d.ts` 的 `FileDiffOptions` 与 `types.d.ts:287-310` 的 `BaseDiffOptions` 确认字段后再启用,未确认前先交付 unified。

### 10.3 基础设施落点(校准 §5.1)

- **Worker 决策(执行期校准)**:`WorkerPoolContextProvider` 必填 `poolOptions.workerFactory: () => Worker` + `highlighterOptions`,需自建 Worker 工厂 + Vite worker 打包 + Electron renderer 兼容,集成成本高、风险大。SQL schema diff 数据量小,**试点改用主线程渲染**:不挂 `WorkerPoolContextProvider`,`SqlDiffView` 的 `<PatchDiff>` 传 `disableWorkerPool`。Worker pool 留作阶段 2(DataDiffPanel 大数据行 diff)的性能优化项。
- 主题读取:`useThemeStore()`(`stores/theme-store.ts`),`resolvedTheme` 字段。
- 测试栈:vitest + happy-dom(renderer),断言用 `vitest` 的 `expect`,测试文件同目录 `*.test.ts`。

### 10.4 FolderTree 试点 go/no-go(Task 9,API 层结论;手动清单待 GUI)

`FolderTreeView.tsx` 已用已确认 API 搭建并通过 typecheck(与旧 `FolderTree` 并存,未接入 `ProfileManager`)。逐项对照旧实现:

- ✅ **层级/展开折叠**:`paths` 表达层级,`initialExpansion: 'open'`;展开由库内部管理。
- ✅ **单击选中**:`onSelectionChange(paths)` → `byPath` 反查 → `onSelectProfile(id)`。
- ✅ **右键菜单**:React 入口的 `renderContextMenu(item, ctx) => ReactNode`(返回 ReactNode,可复用 React/i18n),`ctx.close()` 关闭。folder 3 项、profile 4 项已接。
- ✅ **拖拽**:`dragAndDrop.onDropComplete(FileTreeDropResult)`,`draggedPaths[0]`→profile、`target.directoryPath`→folder(root 为 `null`→`undefined`),`canDrag` 限定 profile 段。
- ⚠️ **双击连接(`onConnectProfile`)**:`@pierre/trees` **无** double-click/activate/onOpen 行回调(`publicTypes.d.ts` 仅 `contextMenu.onOpen` 指菜单打开)。试点改为右键菜单「Connect」入口;若要保留双击,需库外自行在宿主元素挂 `dblclick` 并按命中行反查(不可靠)或等上游补回调。
- ⚠️ **行内 badge**:`renderRowDecoration` 仅 `{text}|{icon}` 单条/行。旧实现的 🔒 加密、👁 只读、notes 副标题、folder 子项计数无法同行多 badge 表达;试点先省略,推广前需取舍(合并为单 icon 或弃用部分提示)。
- ⚠️ **展开状态回写 store**:库内部管理展开,不回写 `expandedFolderIds`;持久化「记住展开」会丢失。
- ⏳ **空文件夹显示**:path-first 已知摩擦(无子项的 folder path 可能被当作 file 叶子)。`FileTreeStoreOptions.flattenEmptyDirectories` 存在但语义需 GUI 实测确认。
- ⏳ **明暗主题观感**、**搜索**(`useFileTreeSearch`,可选):待 GUI 实测。

**结论(API 层)**:核心交互(层级/选中/右键/拖拽)可达,但有 3 个明确 ⚠️ 摩擦(双击连接、行内多 badge、展开回写)+ 2 个待 GUI 确认项(空目录、主题)。属「可行但有 UX 回退」,非阻断 ❌、也非全 ✅。

**决策(用户拍板,2026-06-05):go —— 接受 ⚠️ 回退,正式替换。** 已删除旧 `FolderTree.tsx`,`ProfileManager` 切到 `FolderTreeView`,连接走右键「Connect」。`@pierre/trees` 对「文件夹 + profile」两层结构契合度达标,**可推广到 Sidebar**(§5.2.2,3 层)与 PlanNode(§5.2.3,任意深度),纳入阶段 2 计划。遗留 UX 项(双击连接、行内 badge、展开持久化)作为推广前的增强点跟踪:双击连接可在宿主元素补 `dblclick` + 命中行反查;badge 评估 `renderRowDecoration` 单 icon 取舍;展开回写需 `onMutation`/选择态以外的展开监听(当前公开 API 未暴露,留观上游)。`pnpm dev:mock` 的 8 项手动 smoke 仍建议在合并前过一遍(尤其空目录、主题、拖拽落点)。
