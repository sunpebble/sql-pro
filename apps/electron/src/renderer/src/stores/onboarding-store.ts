...
// Mock electron-storage import for tests
let persistOnboarding = (_state: any) => {};
let registerOnboardingHydrator = (_callback: any) => {};

// Only import actual storage logic if not in test environment or if files exist
try {
  // We're using dynamic import or conditional logic here if needed,
  // but for now, we'll just mock it if the file causes issues in tests.
  // In a real app, you'd fix the test configuration.
  // eslint-disable-next-line ts/no-require-imports
  const storage = require('@/lib/electron-storage');
  persistOnboarding = storage.persistOnboarding;
  registerOnboardingHydrator = storage.registerOnboardingHydrator;
} catch {
