...
export function MemoryMonitorPanel({
  ...
  // Cache stats
  const tableDataStats = useTableDataStore((state) => state.getCacheStats());
  const queryResultsStats = useQueryStore((state) =>
    state.getResultsCacheStats()
  );
  const schemaCacheStats = useMemo(
