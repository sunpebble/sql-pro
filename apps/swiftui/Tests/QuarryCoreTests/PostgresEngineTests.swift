...
//   QUARRY_PG_TEST_PORT=54329 swift test --filter PostgresEngineTests
final class PostgresEngineTests: XCTestCase {
  private func makeEngine() throws -> PostgresEngine {
    guard let port = ProcessInfo.processInfo.environment["QUARRY_PG_TEST_PORT"].flatMap(Int.init) else {
      throw XCTSkip("Set QUARRY_PG_TEST_PORT to run PostgreSQL integration tests.")
    }
    return try PostgresEngine(
      host: "127.0.0.1",
      port: port,
      database: "quarrytest",
      user: "quarry",
